...
export default async function BoardsPage() {
  ...
  const boards = await prisma.board.findMany({ where: { ownerId: user.id }, include: { artworks: { include: { artwork: true }, take: 4 }, _count: { select: { artworks: true } } }, orderBy: { createdAt: "desc" } });
  return <main className="mx-auto w-full max-w-5xl flex-1 px-4 py-8 sm:px-6"><h1 className="text-3xl font-semibold tracking-tight">Your boards</h1><p className="mt-2 text-sm text-stone-600">Save artwork into collections you can revisit.</p>{boards.length ? <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{boards.map(board => <section key={board.id} className="overflow-hidden rounded-2xl bg-white ring-1 ring-stone-200"><div className="grid h-32 grid-cols-2 bg-stone-100">{board.artworks.map(({ artwork }) => <Image key={artwork.id} src={artwork.imageUrl} alt="" width={200} height={160} className="h-full w-full object-cover" />)}</div><div className="p-4"><h2 className="font-semibold">{board.name}</h2><p className="mt-1 text-sm text-stone-500">{board._count.artworks} saved pieces</p></div></section>)}</div> : <div className="mt-8 rounded-2xl border border-dashed border-stone-300 bg-white p-10 text-center text-sm text-stone-600">Open any artwork and use “Save to board” to start a collection.</div>}</main>;
}
